...
let url = "https://www.kkphim.vip";
let urlApi = "https://phimapi.com";
let siteKey = "";
let siteType = 0;
...
async function detail(id: string): Promise<Stringified<VodData>> {
  const data = JSON.parse(await getRequest<string>(`${urlApi}/phim/${id}`));
  const vod = {
    ...
    vod_play_from: data.episodes
      .map((e) => e.server_name.replace(/[^a-zA-Z0-9\\s]/g, ""))
      .join("$$$"),
