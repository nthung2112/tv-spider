...
function lcs(str1, str2) {
  ...
  for (
    ...
    for (var subArray = new Array(str2Length), j = 0; j < str2Length; j++)
      subArray[j] = 0;
    num[i] = subArray;
  }
...
function findBestLCS(mainString, targetStrings) {
  ...
  for (let i = 0; i < targetStrings.length; i++) {
    ...
    results.push({ target: currentTargetString, lcs: currentLCS }),
      currentLCS.length > results[bestMatchIndex].lcs.length &&
        (bestMatchIndex = i);
  }
  return {
    allLCS: results,
...
function showDateText(timestamp) {
