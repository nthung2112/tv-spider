...
async function category(tid, pg, filter4, extend) {
  if (!pg) pg = "1";
  let link = url;
  if (extend.tag || tid) {
    link = `${url}${extend.tag || tid}?page=${pg}`;
  }
  const data2 = await getRequest(link);
  const $2 = load(data2);
  const items = $2("table tbody tr");
...
async function detail(id) {
  ...
  let vod = {
    ...
    vod_name: `${data2.movie.name} (${data2.movie.original_name})`,
    vod_director: data2.movie.director,
    vod_actor: data2.movie.casts,
    vod_content: data2.movie.description,
    vod_play_from: data2.movie.episodes.map((e) => e.server_name).join("$$$"),
    vod_play_url: data2.movie.episodes.map((e) => e.items.map((d) => `${d.name}$${d.m3u8}`).join("#")).join("$$$")
  };
  return JSON.stringify({
    list: [vod]
  });
