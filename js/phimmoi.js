...
async function detail(id) {
  ...
  const $ = load(html);
  const playDetailUrl = $("ul a.btn-see")[0].attribs["href"].replace(
    "/info/",
    "/xem/"
  );
  let vod = {
    vod_id: id,
    vod_pic: $("#detail-page > div.film-info > div.image > img")[0].attribs[
      "src"
    ],
  ...
  if (listItem.length > 0) {
    const allItem = _.map(
      listItem,
      (item) => `${$X(item).text().trim()}$${item.attribs.href}`
    );
