...
async function detail(id) {
  ...
  const episodes = JSON.parse(
    await request(`${url}/api/v2/films/${id}/episodes?sort=name`)
  );

  vod.vod_play_url = episodes.data
    .map((d) => `${d.full_name}$${id}/episodes/${d.id}`)
    .join("#");
...
async function play(flag, urlId, flags) {
  ...
  const result = data.sources.m3u8.opt
    ? decode_1080(data.sources.m3u8.opt, 69).trim()
    : "";
  return JSON.stringify({
    parse: 0,
    url: result,
...
async function search(wd, quick) {
  const videos = JSON.parse(
    await request(url + `/api/v2/search?q=${wd}&limit=20`)
  ).data;
